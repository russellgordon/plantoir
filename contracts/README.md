...
| The spellings of "deploy at &lt;time&gt;" that are answered in code, the ones that go to the model, and which day a bare time means | `assist-cases.json` → `deployAtATime` | ScheduleDeployCard (6). AUTHORED, and the one `cardPhrasings.parsed` cannot carry: a family whose variable part is a TIME needs its spellings written out, or one example becomes one spelling |
...
| What a removal does to the marks pool | `shared-rules.json` → `gradedFolders.removingAFolder` | `GradedFolderChoices` (6 cases), played through Course Settings in the order it really happens. Proposed FROM the mac 2026-09-09 and **run on both platforms since 2026-09-18** ([#142](https://github.com/russellgordon/plantoir/issues/142), where Russell settled that Windows adopts the rule): Windows plays the same six through one Core method, `FolderRemoval.RemoveFolderFromCourse`, because the ORDER is the rule and an order left in the view can be pinned by no test there. Four of the six used to fail there; putting that body back still turns exactly those four red. |
...
| `gradedFolders.removingAFolder` — what a removal does to the marks pool, all six cases played through `FolderRemoval.RemoveFolderFromCourse` (adopted from the mac 2026-09-18, [#142](https://github.com/russellgordon/plantoir/issues/142)) | `GradedFolderChoicesTests.WhatARemovalDoesToTheMarksPoolMatchesTheContract`. One thing the six cases cannot see is matching CASE, which Windows asks insensitively so that it agrees with the build; that half is proposed to the mac as [#172](https://github.com/russellgordon/plantoir/issues/172) and pinned locally by `ANameStillOfferedInANOTHERCasingKeepsItsPlaceInThePool` |
...
open issue, or exempt for a reason written down here.** **111 case lists; 100
have a reader here.** The other eleven are below.
