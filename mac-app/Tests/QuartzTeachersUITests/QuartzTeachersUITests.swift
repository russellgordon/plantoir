...
final class QuartzTeachersUITests: XCTestCase {
    ...
    /// is still running. The work is `StubLaunchers.reap`, shared with the
    /// other UI test classes.
    ...
    /// The short name of a running process — `StubLaunchers`'.
    func nameOfRunningProcess(_ processID: Int32) -> String? {
        return StubLaunchers.nameOfRunningProcess(processID)
    }

    /// Writes a fake `preview.sh` — `StubLaunchers.writeStubPreviewScript`,
    /// remembered here so `tearDown` reaps the server it starts.
    ...
    ///
    /// Through `IsolatedLaunch`, so the app keeps its trail and preferences
    /// in a state folder of its own rather than the teacher's (#154).
    func launchApp() throws -> XCUIApplication {
        return try IsolatedLaunch.launch().application
    }
