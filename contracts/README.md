...
| How a name in `excluded_items` is matched, and when a removal or an add-back is written on the trail ([#152](https://github.com/russellgordon/plantoir/issues/152)) | `shared-rules.json` → `excludedItems.matching`, `.recordedOnSave` | `matching`: `scripts/test_preflight_exclusions.py` → `test_every_matching_case`, through the real preflight AND `_dropping_excluded_items` (E2, E4 red on the give-up path before the fix), and `ExcludedItemsContractTests.testAnExcludedNameIsMatchedAsTheContractSays` through `isExcluded`. `recordedOnSave`: `ExcludedItemsContractTests.testWhatAWriteRecordsMatchesTheContract` (8 cases through `ExclusionTrail.changes`) plus five trail tests in `GradedFolderChoicesTests` through the real page and `SectionAdder`. AUTHORED, arrived 2026-09-26. |
...
(the marks floor, exact exclusions, a chosen pool's spelling, and exclusion
lines written with the file), counted ON THIS BRANCH (off `dev` ff1213ed, which
read **228**). The walker reads **232**: four new lists, all in
`shared-rules.json` — `gradedFolders.floor.cases` (17) and
`excludedItems.recordedOnSave.cases` (8), owed by the `windows` issue from #152
...
`activityTrail.mustRecord` is unchanged at **83**: `item excluded` and `item
re-included` changed WHEN they are written and gained the kind, so their
`carries` and `why` changed. `shared-rules.json` holds **thirty-six** rule sets
...
`activityTrail.mustRecord` **91** (dev's, unchanged by #152), and
...
| `shared-rules.json` → `excludedItems.recordedOnSave.cases` | 8 | **Owed**, the same issue. Their `RecordExclusion` / `RecordReInclusion` write on the click, as the mac did; the cases are a pure comparison (before, written → events, scopes, kinds, names), runnable the moment the comparison exists, and the words of the lines are deliberately not pinned. |
