...
**What the mac still owes is no longer in this file.** Work moved to [GitHub
issues](https://github.com/russellgordon/plantoir/issues) on 2026-09-08: a
...
## How to write an issue for the mac

The good entries below already do this; it is written down so the next one
does not have to infer it from twenty examples. **Open it as a GitHub issue
labelled `mac`** — `decision` as well if it needs Russell to choose — and give
it a milestone. Whichever it is, it carries:
...
5. **Whether the mac is expected to match it, or merely to know.** Those are
   different asks. Something the mac need only KNOW is not an issue at all —
   it goes in "For awareness" below.
...
- **Windows now clicks the New Course wizard's Create button in a running app,
  ...
  **And the half Windows did NOT automate, with the reasoning, so nobody
  ...
  same omission. The one thing the mac is actually ASKED to do is a GitHub
  issue labelled `mac`, rather than buried here.
...
- **Windows now spells a renamed folder the way Quartz reads it — item 31's
  ...
  changes, and one new contract case that should already be green here.
...
- **Anything you build over there now owes a trail line** (mac, 2026-08-16).
  ...
  records it — that is the mechanism working, not damage — so open a `mac`
  issue naming the case and it reads as a request. The
