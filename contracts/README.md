...
| Adding a section: suggestion, refusals, wording — and the keys it adds to every course-level page, whatever the page's fence and line endings (#175) | `course-management.json` → `sectionNumbers` (`addingKeysToAPage` for the last) | SectionAdder, SectionNumbersValidation (21); `CourseManagementContractTests.testAddingASectionsKeysToAPageIsWhatTheContractSays` (6 cases, compared as bytes) |
...
at `origin/dev` (9d4bf2a6) and **172** after. The one new list is
`course-management.json` → `sectionNumbers.addingKeysToAPage.cases` (6), with
...
| `course-management.json` → `sectionNumbers.addingKeysToAPage.cases` | 6 | **Owed**, the `windows` issue opened from [#175](https://github.com/russellgordon/plantoir/issues/175). AUTHORED, arrived 2026-09-25. Whole-file before/after pairs: Windows' `SectionAdder.FrontmatterLines` is strict (`lines[i].TrimEnd('\r') == "---"` on line 0), so four of the six skip the page today — two of them publishing a page hidden in section 1 into the new section. Unrun rather than failing; wiring them in is the acceptance. |
