...
| `gradedFolders.rerunningSetup` — a re-run of course setup writes a saved marks pool back as it was, all ten cases ([#192](https://github.com/russellgordon/plantoir/issues/192), 2026-09-25) | `PythonToolchainTests`, which DISCOVERS `scripts/test_graded_folders_rerun.py` — no reader to write. The test replaces `input` and `getch` in-process and needs no terminal; it has not yet been run on Windows paths, and one `dotnet test` confirming it green is the whole of what is owed. |
...
one new list, `shared-rules.json` → `gradedFolders.rerunningSetup.cases` (10),
