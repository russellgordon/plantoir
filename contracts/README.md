...
none is a new list: `readingALink.cases` (10 → 34), `readingALink.rejected`
...
| `shared-rules.json` → `readingALink.cases` | 34 | **Owed**, the same `windows` issue from #294 — distinct `WikiLinks.Parse` targets must equal `expect`. The build half is run there already, through `PythonToolchainTests` discovering `test_dates_follow_the_class.py`. Pure; seven of the ten are red against Windows' current pattern by reading, not by running. Since #313 (2026-09-26) 24 more cases say where code is; by emulation (their blanking stripper, reproduced in Python), 8 of the first 22 are red against Windows' `WithoutCode` (the 5th, 6th, 8th, 9th, 10th, 12th, 17th and 21st new cases), and the last two were added so that a reader closing a fence on either character, or not seeing a fence inside a callout, goes red (their `whatIsCode` implementation is owed by the `windows` issue from #313). |
