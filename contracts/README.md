...
| When quitting asks the teacher first, and when it must NEVER ask | `shared-rules.json` → `quittingWhileWorkIsUnderWay` | QuitConfirmation (4) on the mac. **`appliesOn: ["mac"]` for now, and deliberately not for ever**: added 2026-09-19 with [#220](https://github.com/russellgordon/plantoir/issues/220), when nobody was working on Windows. Its five cases are platform-neutral apart from the signal that says the system is logging out; **no Windows reader today**, and the four matching `activityTrail.mustRecord` events carry the same `appliesOn` so their suite is not red on pull. Adopt the rule and the trail line together, then drop the `appliesOn` |
...
| `shared-rules.json` → `quittingWhileWorkIsUnderWay.cases` | 5 | **No reader on either platform yet**, and the newest of the twenty-one. Arrived 2026-09-19 with [#220](https://github.com/russellgordon/plantoir/issues/220); the MAC runs them (`QuitConfirmationTests.testTheRuleIsTheOneTheContractWritesDown`), Windows has no such question, and the block carries `appliesOn: ["mac"]` as an acknowledged exception — see "One exception has been taken to the passage above" under Named gaps. The five cases are platform-neutral apart from the two `quitReason` spellings; the discriminating one is "a preview is open and nothing is publishing", which pins that a preview merely being OPEN is not work under way. Owed with the behaviour, in the `windows` issue that carries #220's handover |
