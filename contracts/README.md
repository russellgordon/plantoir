...
| How a date or title is rewritten when its value continues below the key, and what the site then reads (#199) | `file-formats.json` → `datesAndTitles.writingCases` | `FileFormatsContractTests.testTheDateAndTitleWritersTakeAKeysWholeValue` (10 cases, bytes); `scripts/check_dates_and_titles_against_the_site.py` in the image (verify.sh) checks every `expectSiteReads` against the real `process_frontmatter` |
...
list, `file-formats.json` → `datesAndTitles.writingCases.cases` (10), with no
...
the same base; the merge that lands second re-counts.
...
| `file-formats.json` → `datesAndTitles.writingCases.cases` | 10 | **Owed**, the `windows` issue opened from [#199](https://github.com/russellgordon/plantoir/issues/199). AUTHORED, arrived 2026-09-25. Windows' `PageFrontmatter.SetTitle` and `SetCreated` replace the key's line alone (`lines[at] = ReplaceRawValue(...)`), so the seven non-control cases fail there until they call their own `ContinuationLines`. Unrun rather than failing. |
