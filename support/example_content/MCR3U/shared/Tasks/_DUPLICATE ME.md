...
watch them do, and what they tell you. The first arrives on its own. Write the
...
OBSERVE — Unit ?, Day ?, which period
  ...
  Watch for: something visible only while they work, and gone by the time the
  page is handed in. If you cannot name one, the task may be worth rethinking.

  Going well:

  Stuck:

  Record: how you will note it in a few seconds, for a whole room.

TALK — Unit ?, Day ?, ideally a conference the timetable already has

  Ask: a real question, in the words you would use, and not one already printed
  where the students could have rehearsed it.
...
The product evidence is: what is handed in, and when.
%%
