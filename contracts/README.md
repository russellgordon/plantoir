...
**181** after: `deployAtATime.sayItAs` (36) is new, with no Windows reader, so
it joins the `deployAtATime` row in the table below. `deployAtATime.asked`
went from 11 to 21, `.refused` from 28 to 53 (`deploy at 6.30 pm` moved to
`sayItAs`; 26 must-not-catch rows added) and `scenarios.cases` from 16 to 18 —
rows, not lists. `activityTrail.mustRecord` is unchanged at 73: the event
reused is `assistant matched a fixed phrase`, whose `carries` now says it. The
wording block gained `sayTheTimeAs`, `sayTheTimeAsWithoutTheComma` and
`sayTheTimeAsWithoutPlease` — three renderings of one function, see the
