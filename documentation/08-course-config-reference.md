...
| `include_curriculum_coverage` | bool (default `true`) | setup | build | Whether the generated coverage maps are produced — ONE switch for every map (#128): `Curriculum Coverage` and one `<Folder> Coverage` per further declared curriculum folder. A club (#267) is created with it `false` and no Curriculum folder; a teacher who turns it on later in Course Settings gets the build-health advisory `curriculumCoverageFoundNothing` until a curriculum folder with pages exists — advisory and correct, not a fault. |
...
| `curriculum_folder` | string or null | LEGACY — written by setup before #128; now only REWRITTEN by a rename that renames its folder, never created | build, both apps | One curriculum folder's name. Still read forever, unioned after `curriculum_folders`, so a folder an older Plantoir wrote down still counts. Setup no longer writes it — it used to write it from the manifest on every run, which put `Curriculum` back over a name a rename had recorded. |
...
entries, read from the source rather than from a list of names. The cases are
`shared-rules.json` → `referenceCourses.importing.quartzCheckoutLayout.placement`;
the reasoning is [09 → "The 2024–25 layout"](09-mac-app.md).
...
That is also why each app writes a NEW course's pool itself: setup runs over the
file the app has just written, reads it as answered, and leaves a key the app
left out absent — which is how every course made in the apps from ready-made
pages came to read as never asked (`gradedFolders.newCourse`, #292).
...
- `prepopulate_example_content`, `include_curriculum_pages` (entries
  ...
  exists for the course code. Since #251 they are no longer written
  together: a teacher who declines the payload and keeps the subject's
  skeleton gets `prepopulate_example_content: false` beside
  `include_curriculum_pages: true`, which is what fetches the
  expectations written for their code
...
- `include_curriculum_coverage` and `include_coverage_notes` (entries 125,
  130) — whether the generated coverage map pages are produced (every map
  one per declared curriculum folder since #128 — one switch for all), and
