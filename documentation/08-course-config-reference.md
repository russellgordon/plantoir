...
| `<CODE>/.netlify_sites/section<N>.previous-<stamp>.json`<br>`<CODE>/.cloudflare_sites/section<N>.previous-<stamp>.json` | A marker set aside when a section was **rolled over onto a new website** — the assistant renames it rather than deleting it, because it holds the site id and admin address and is the only way back to last year's site. Inert: the scripts build exact marker paths and never scan the folder. See `contracts/file-formats.json` → `firstDeployMarkers.releasedWhenASectionRollsOver`. |
