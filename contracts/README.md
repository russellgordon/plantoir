...
(what backups take, and deleting several). The walker read **171** at
`origin/dev` (45a64942) and reads **174** after: `course-management.json` →
`backups.pruneCases` (3), `.sizeCases` (2) and `.deleteCases` (3), none with a
Windows reader, all in the table below; and `activityTrail.mustRecord` grew
from 70 to 71 (`backups deleted`), which is not a new list.
