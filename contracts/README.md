...
none is a new list: `readingALink.cases` (10 → 39), `readingALink.rejected`
...
Re-take both at the merge if anything else landed first.
...
|---|---|---|
| `class-planning.json` → `renamingTheUnitWord.cases`, `.linkCases.cases` | 7 + 5 | **Owed**, [#158](https://github.com/russellgordon/plantoir/issues/158) (v1.3.0) — Windows cannot rename a course's word for a unit at all yet. Unrun rather than failing; the paragraph above says why that is the quiet kind of gap. |
...
| `shared-rules.json` → `followingLinks.publishing.cases` | 2 | **Owed**, the `windows` issue opened from [#294](https://github.com/russellgordon/plantoir/issues/294). AUTHORED, arrived 2026-09-26. **Unrun rather than red**: `SharedRules_FollowingLinks_MatchesContract` asserts the three booleans and walks no `cases` array. What they owe is the loop, through a real publish, and the reader fix it would expose: `WikiLinks.LinkPattern` reads `[[Ohm's Law\|Ohm]]` as `Ohm's Law\`, which `Resolve` then treats as a path and reports NotFound, so the case is RED by behaviour there. `publishing.casesNote` defines `body` (the page's text exactly, replacing the `See [[x]].` paragraphs `links` would write; never both). |
| `shared-rules.json` → `readingALink.cases` | 39 | **Owed**, the same `windows` issue from #294 — distinct `WikiLinks.Parse` targets must equal `expect`. The build half is run there already, through `PythonToolchainTests` discovering `test_dates_follow_the_class.py`. Pure; seven of the ten are red against Windows' current pattern by reading, not by running. Since #313 (2026-09-26) 29 more cases say where code is; by emulation (their blanking stripper, reproduced in Python), 8 of the first 22 are red against Windows' `WithoutCode` (the 5th, 6th, 8th, 9th, 10th, 12th, 17th and 21st new cases), and the seven after them were added while implementing — so that a reader closing a fence on either character, not seeing a fence inside a callout, running a span on from a heading or into a callout line, goes red, and to pin a fence ending with its callout, a quoted line inside an unquoted fence, and frontmatter's closing `---` (their `whatIsCode` implementation is owed by the `windows` issue from #313). |
| `shared-rules.json` → `readingALink.rejected` | 6 | **Exempt: prose with fields.** Six rejected designs (#313 replaced one — the deferral of code, now decided — and added three) with their reasons — counted because the walker counts an array of objects. |
