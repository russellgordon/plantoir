...
| `metal-routing-results.txt` | **Routing on the hardware that ships, after the 2026-08-24 prompt change** (issue #117). Both tiers, three system-prompt forms including the one before the change, the app's own request body, and the mac's OWN shelf — the "promise card" in every other file here is Windows' `ExampleRequests`, not this app's. The answer on the prompt: neutral on the 4B (28 of 29 probes identical either way), and worth 5 trials in 10 on one hide request on the 1.5B. The findings that matter are elsewhere in it, and each is now an issue: `read` on the 4B has gone from 10/10 to 0-1/10 since August (#167), the mac shelf's "Deploy at 6:30 AM" deploys NOW on the small tier (#168 — **closed in code on 2026-09-19**, see `metal-routing-postscript-168.txt` beside it: the sentence is matched in `AssistCardCommand` and no longer reaches the model, so the shelf's split went to 16 answered in code and 3 routed — and to **17/2** later the same day, when [#215](https://github.com/russellgordon/plantoir/issues/215) made "hide" and "unpublish" one frame that takes a class page; count it in `AssistPromptShelfTests.testEveryCardIsEitherMatchedInCodeOrKnownToGoToTheModel` rather than from any prose. The measurement is unchanged and still true OF THE MODEL), and the app sent no `max_tokens` (#166, since fixed — so **this file's `--app-body` arm is `--app-body --uncapped` today**, and a re-run without that flag is measuring a different body). One more sentence in it went out of date on 2026-09-19: it says the app "rewrites the course and section" before a call runs, and since [#202](https://github.com/russellgordon/plantoir/issues/202) a wrong COURSE is REFUSED rather than rewritten — so its wrong-course counter is now LESS strict than the product rather than more. See `metal-routing-postscript-202.txt` beside it, which also carries the per-file count behind that decision (686 responses, 0 wrong course codes). Checked by three Claude Opus reviews in fresh contexts — of the plan, the results and the corrections — and a final Claude Fable 5.1 pre-merge sweep. Also the file to read before quoting any "malformed tool calls: 0" in this folder. Pre-registered: its thresholds were committed before the first model call. |
...
| `echo-postscript-215.txt` | **What the smaller assistant did with the word "hide", and what ONE echo did to the rest of the conversation** (issue #215, 2026-09-19). Five phrasings of "hide <page>" chose no tool at all and replied with the teacher's own sentence, date line and all, while the same request worded "unpublish" was right every time — identical with and without the request cap, so it is not new. Worse, replayed on two courses and two pages: after one echo, "Unpublish Unit 4, Day 20" echoed too, a sentence a FRESH conversation gets right every time. It is the HISTORY, not the sentence. Carries the conditions, what was done in code, what was REJECTED and why (a tool-description sentence, a publish mirror, `show`/`unhide`, folding the trail line into the cut-off event, a fuzzy echo rule), and the re-counted cost to this folder's denominators: 6 of 29 probes are answered in code where it was 5, and the promise-card line the runs print drops from 6 of 11 to 5 of 11. The 4B was NOT measured — it is not downloaded on this Mac. A postscript rather than an edit: no results file was changed. |
| `link-question-results.txt` | **"What does this page link to?", on both tiers, across courses AND dates** (issue #167, 2026-09-25). The 29-probe suite scored `read` 10/10 on the larger assistant that day (290/290 overall) and 0/10 on the smaller — and that 10/10 is the date lottery #167's own comments had already described: greedy decoding makes ten trials of one date one measurement. The grid beside it is the measurement that matters: six courses × fourteen dates, one greedy trial per cell, seven phrasings. "What does Unit 2, Day 3 link to?" is **0 of 84 on BOTH tiers**; "Which pages does Unit 2, Day 3 link to?" became a `publish_pages` plan 31 times in 84 on the smaller one; the family scored 153/588 and 115/588. Raw output beside it (`link-question-29-{large,small}.txt`, `link-question-grid-{large,small}.txt`) and the grid's script, `link-question-grid.py`. What was done with it — the ninth parsed family, answered in code with no byte of the tool surface moved — is its postscript, which also re-counts this folder's denominators: 7 of 29 probes answered in code where it was 6, so "the N the model SEES" drops from 23 to 22 (on the mac; Windows' model still sees `read` until it answers the family in code too). |
| `two-lap-all-results.txt` | **What the smaller assistant writes for "publish all of those", and what #197 did about it** (2026-09-26). The shape `token-cap-results.txt` measured as `"pages": "all"` 3/3 on one real course was re-run on six example payloads × two code spellings × three dates × three follow-ups — **0 of 108 answered "all"**; all 108 were an invented list cut off at the cap, which `answerWasCutOff` already refuses. So the shape fails two ways depending on the listing, and the "all" half was the unhandled one (the app said "Nothing needed changing."). After the fix: "Publish all the classes in Unit 2." still goes to `publish_class_on` with today's date 3/3 AS THE MODEL ANSWERS IT (the surface did not move), and the app now answers it in code; the same sentence naming its course still misroutes 3/3 and is recorded as not fixed. Also the plan-mode lap after the new refusal (a text list cut off at the cap, 3/3 — a lap the window never takes). Harnesses beside it: `two-lap-all.py`, `two-lap-all-grid.py`, `pages-naming-no-page-probe.py`. Smaller tier only. |
| `description-convergence-results.txt` | **#114's three arms on the smaller assistant** (2026-09-26): the contract's descriptions (A), Windows' router text today — `Briefly()` of its own descriptions (B), and `Briefly()` of the contract's text (C), differing only in the descriptions. A and B tie on the 29-probe suite (160/220 the model sees) but B runs one probe to the cap 10/10, which the suite scores as a pass; on teachers-say A is 210/250 against 180/250; C loses 40 control trials against A. 0 polarity inversions in any arm. The mac's reference for the direction of the decision, not the Windows measurement it requires. Arm builder beside it: `description-arms.py`, checked byte-identical to the arms measured. Smaller tier only; the larger was not on the Mac. |
