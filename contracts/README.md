...
| Whether a built site is a PREVIEW's — any page under `public/` carrying the live-reload client, not the front page alone (#136) | `app-rules.json` → `buildFreshness.previewBuild` (`signature`, `where`, `notShared`, `cases`) | BuildFreshness (3: every case, the signature, and a mixed-state course laid out behind a `.merged_output` link), `ScheduledPublishOutcomeTests` (the overnight run's own shell against every case, a build question behind a clean front page, an unreadable front page — 3), and `scripts/test_preview_build_detection.py` (3 — `deploy.sh`'s check cut out of the launcher and RUN, and the real `deploy.py` run, against every case; verify.sh and Windows' `PythonToolchainTests`, which skips the bash half where there is no bash and the two `unreadable` cases where a page cannot be made unreadable). Windows' own `BuildFreshness` reads the front page alone, and `deploy.ps1`'s `Test-CarriesLiveReload` is not run against the cases — both owed, see the census below |
...
| What publishing and unpublishing do to linked pages, and what is never swept | `shared-rules.json` → `followingLinks` | SharedRulesContract (3, plus #294's `testThePublishingCasesSayWhatABodyIs`), AssistToolRunner (9, plus #294's `testPublishingFollowsLinksAsTheContractSays`, which shares one harness with the `stopsAtAClassPage` loop). Its `stopsAtAClassPage.cases` (3) arrived 2026-09-19 with [#173](https://github.com/russellgordon/plantoir/issues/173), and `publishing.cases` (1) — links written in a TABLE, with the alias pipe escaped, followed through a real publish — arrived 2026-09-26 with [#294](https://github.com/russellgordon/plantoir/issues/294); `publishing.casesNote` says what a page's `body` means, so the harness can be built from the contract alone. Neither has a Windows reader — see the census table below. The three `publishing` booleans stay TRUE and both suites assert them: the walk is still transitive, it has one stop |
| What a wikilink NAMES — including the escaped pipe `[[Page\|words]]` Obsidian writes for an alias inside a table, and a heading followed by an alias — one definition for every reader | `shared-rules.json` → `readingALink` (`rule`, `whenRewritten`, `expectIs`, `why`, `rejected`, `cases`) | SharedRulesContract (1, `testEveryLinkShapeReadsAsTheContractSays` — every case through BOTH mac readers, `linksAsWritten` and `linkTargets`) and `scripts/test_dates_follow_the_class.py` (`test_every_link_shape_reads_as_the_contract_says`, through the build's `_extract_wikilink_targets`; in the image under verify.sh, and in Windows' `PythonToolchainTests`). AUTHORED, arrived 2026-09-26 with [#294](https://github.com/russellgordon/plantoir/issues/294). Windows' own reader, `WikiLinks.Parse`, is not run against it — owed, see the census table below. Pure: text in, names out |
...
| Phrasings matched in code, including the PARSED families (nine since [#167](https://github.com/russellgordon/plantoir/issues/167) — count `parsed` rather than trusting this number) | `assist-cases.json` → `cardPhrasings` | AssistContract (1), AssistPromptShelf (2), AssistToolRunner (4), AssistScenario (1, walking `parsed`) |
...
**230**. The walker reads **232**: two new lists, both in `app-rules.json` →
`helperBootstrap` — `installCases` (18) and `firstStartCases` (6) — exempt
permanently (`appliesOn: ["mac"]`) and in the table below.
`activityTrail.mustRecord` grew from 91 to **93** ("helper programs installed"
and "website builder created", both `appliesOn: ["mac"]`), which is not a new
list and asks nothing of Windows. `shared-rules.json` still holds
**thirty-six** rule sets; `app-rules.json` gains one top-level key.
...
|---|---|---|
| `class-planning.json` → `renamingTheUnitWord.cases`, `.linkCases.cases` | 7 + 4 | **Owed**, [#158](https://github.com/russellgordon/plantoir/issues/158) (v1.3.0) — Windows cannot rename a course's word for a unit at all yet. Unrun rather than failing; the paragraph above says why that is the quiet kind of gap. |
...
| `shared-rules.json` → `followingLinks.publishing.cases` | 1 | **Owed**, the `windows` issue opened from [#294](https://github.com/russellgordon/plantoir/issues/294). AUTHORED, arrived 2026-09-26. **Unrun rather than red**: `SharedRules_FollowingLinks_MatchesContract` asserts the three booleans and walks no `cases` array. What they owe is the loop, through a real publish, and the reader fix it would expose: `WikiLinks.LinkPattern` reads `[[Ohm's Law\|Ohm]]` as `Ohm's Law\`, which `Resolve` then treats as a path and reports NotFound, so the case is RED by behaviour there. `publishing.casesNote` defines `body` (the page's text exactly, replacing the `See [[x]].` paragraphs `links` would write; never both). |
| `shared-rules.json` → `readingALink.cases` | 10 | **Owed**, the same `windows` issue from #294 — distinct `WikiLinks.Parse` targets must equal `expect`. The build half is run there already, through `PythonToolchainTests` discovering `test_dates_follow_the_class.py`. Pure; seven of the ten are red against Windows' current pattern by reading, not by running. |
| `shared-rules.json` → `readingALink.rejected` | 4 | **Exempt: prose with fields.** Four rejected designs with their reasons — counted because the walker counts an array of objects. |
...
| `course-management.json` → `sectionNumbers.addingKeysToAPage.cases` | 8 | **Owed**, the `windows` issue opened from [#175](https://github.com/russellgordon/plantoir/issues/175). AUTHORED, arrived 2026-09-25. Seven whole-file before/after pairs. Measured by reading `SectionAdder.cs:190-242` (review of #175): Windows' finder wants `---` on line 0 but tolerates CRLF, so THREE of the shapes are skipped (`----` — publishing a page hidden in section 1 — a blank line before, a trailing space); the CRLF case gets its keys but the file is rewritten with LF (`Replace("\r\n","\n")`), so it fails the byte comparison for that reason; and the seventh (a setting below its key) is split the way #181 described. The eighth ([#188](https://github.com/russellgordon/plantoir/issues/188), 2026-09-25) is section 1's value running into a line of INDENTED dashes, which needs #188's fence rule as well. `ExtendFrontmatter` is private there, so running the cases needs it exposed. Unrun rather than failing; wiring them in is the acceptance. |
| `course-management.json` → `backups.restoringOneSectionsKeys.cases` | 6 | **Owed**, the `windows` issue from [#182](https://github.com/russellgordon/plantoir/issues/182) (which folds in [#177](https://github.com/russellgordon/plantoir/issues/177)). AUTHORED, arrived 2026-09-25. Six whole-file restores of one section's keys, each with the site's verdict per section and whether the restore must count the page as not put back. Windows' `CourseRestorer` finds the block with its own strict `FrontmatterBounds` and copies key LINES only, so five of the six fail there until it adopts the shared finder and `LinesOwnedByKey`; the sixth is the guard. Unrun rather than failing. |
| `file-formats.json` → `datesAndTitles.writingCases.cases` | 16 | **Owed**, the `windows` issue opened from [#199](https://github.com/russellgordon/plantoir/issues/199). AUTHORED, arrived 2026-09-25. Windows' `PageFrontmatter.SetTitle` and `SetCreated` replace the key's line alone (`lines[at] = ReplaceRawValue(...)`), so the ten non-control cases fail there until they call their own `ContinuationLines`; the fourteenth ([#188](https://github.com/russellgordon/plantoir/issues/188)) also needs #188's fence rule, and the fifteenth ([#186](https://github.com/russellgordon/plantoir/issues/186)) `SetCreated` declining where there is no place for a new key. Unrun rather than failing. |
| `shared-rules.json` → `scheduledDeployCancellation.cases` | 15 | **Owed**, the `windows` issue opened from [#236](https://github.com/russellgordon/plantoir/issues/236), and since 2026-09-25 the one from [#237](https://github.com/russellgordon/plantoir/issues/237), whose five cases (scheduling and removal with two working folders holding one section, and with a job set before the update) are the acceptance for naming the task after the working folder. AUTHORED, arrived 2026-09-20. **Half of it is already their behaviour and half is a fault they share**: `CourseArchiver.cs` cancels on removing a SECTION and not on removing a COURSE, and their task name is the course code and section only, so the two-working-folders case is theirs too. Unrun rather than red — nothing on that side enumerates the top-level keys of `shared-rules.json`, so a new one is silently ignored, the same precedent as `gradedFolders.removingAFolder` and `workingFolderSelection`. Read `howToRunACase` before wiring it: six cases are played through a real removal, rename or rollover and three are a scan of a source file that must not have learned to cancel. The ninth — the rollover — exists because that path was the ONE cancel left folder-blind when the rule first landed here, which is what a case list is for. |
| `shared-rules.json` → `reopeningTheLastWorkingFolder.launchCases`, `.folderCases`, `.memoryCases`, `.rejected` | 7 + 11 + 3 + 10 | **Owed**, the `windows` issue opened from [#311](https://github.com/russellgordon/plantoir/issues/311). AUTHORED, arrived 2026-09-26. Unrun rather than red — a new top-level key is silently ignored there, the `workingFolderSelection` precedent. Cases carrying `appliesOn: ["mac"]` (the Trash, bookmarks, a log-in restore, the home-folder limit) are skipped there by design; `launchCases` is a pure play of which window gets which folder, `folderCases` needs a real throwaway folder per case (`howToRunACase`), and `rejected` is prose with fields. |
| `shared-rules.json` → `workingFolderReach.pathCases`, `.diskCases`, `.rejected` | 10 + 7 + 9 | **Exempt, permanently: `appliesOn: ["mac"]` on the key and on its trail event.** Windows builds natively, so no folder it can read is out of the builder's reach. Arrived 2026-09-26 with [#290](https://github.com/russellgordon/plantoir/issues/290). If Windows ever builds in a virtual machine again, drop the `appliesOn` rather than adding a twin (`appliesOnWhy`). |
...
| `app-rules.json` → `previewPorts.whenTheWorkspaceIsInUse.whatCountsAsRunning.cases`, `.sequences.cases` | 14 + 6 | **Exempt, permanently: there is no workspace on Windows** (`appliesOn: ["mac"]` on the key, and on its trail event). Arrived 2026-09-25 with [#94](https://github.com/russellgordon/plantoir/issues/94). Read by `scripts/test_port_blocks.py`, which `PythonToolchainTests` discovers on Windows — its bash half skips there, so this is unrun rather than red, and nothing is owed |
