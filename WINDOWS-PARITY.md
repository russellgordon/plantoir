...
**Read this first**, then `WINDOWS-BOOTSTRAP.md`. It does not replace the
bootstrap's rules, and it does not replace the issues. It is the ORDER of the
work, the reason for that order, and the traps between the pieces, so that
forty-six issues read as one piece of work instead of a pile.
...
3. **`PythonToolchainTests` passes every shared `scripts/test_*.py`.** A file
   ...
   no bash). The "known failing" list in `PythonToolchainTests.cs` stays
   empty.
...
7. **Every issue is closed with a closing comment** saying what landed and
   ...
   an open `mac` issue (section 7). The x64 build is ready at "PT - Dev".

---

## 2. What Windows inherits FREE from the shared Python

These pieces were built in `scripts/`, so a Windows build from `dev` already
runs them. **Nothing needs re-implementing.** What is owed is confirming them:
...
| #192 (setup re-run keeps the marks pool) | `setup_course.py` writes a saved `graded_folders` back unchanged | the new shared test named in the #192 draft (`test_graded_folders_rerun.py`) | One run on Windows paths (it has never run there), plus one wrong comment in `NewCourseDialog.cs` (~:1348–1361) |
| #136 (a preview's build is detected across the whole tree) | `deploy.py`'s detection | `test_preview_build_detection.py`. Its two `unreadable` cases skip on Windows | `BuildFreshness.BuiltForPreview` and `deploy.ps1`'s `Test-CarriesLiveReload` (folded into #272) |
...
| #265 (sidebar hide rule v2, build-start marker) | the build never changes `hidden`; filter v2; `ensure_sidebar_hide_rule_current` repairs old sections (**on Windows it runs for real**, because `%TEMP%\quartz-builds` persists there); `.build-started.pending` → `.build-started` | `test_sidebar_hiding.py`, `test_build_started_marker.py` | #272: re-run `Vendor/fetch-runtime.ps1`, `BuildFreshness`, the two-window rule |
...
| #234 (probe the forward before building) | nothing to run: `preview.ps1` has no forward | `test_preview_reach.py`. Its text half must be GREEN | KNOW only |
| #176 (continuation lines) | `setup_course.per_section_frontmatter` steps over a comment at any indent | `CourseLevelSplitterTests` in the shared tests | #190: three stale claims to correct |
...
`68214a6c`, and #192, #136 and #234 each add one. Windows' own
...
- **Config keys.** A string search of `CourseConfiguration.cs` does not find
  ...
  `use_skeleton` are also absent from that one file, but the wizard writes
  them. Check whether the test reads them there.)
...
| # | What it is | Kind | Where the rule lives |
|---|---|---|---|
| **#238** (incoming) | Trail lines are dropped when two processes write at once: `File.AppendAllText` opens with `FileShare.Read`, and an empty `catch` swallows the sharing violation. **Measure first** (kept X of Y, on what hardware), then fix with `FileShare.ReadWrite` plus a single `Write`, or a named `Mutex`. No retry loop. **Do this first in the phase**, because every event below is written through this path | MATCH in effect | doc 09 → "Two writers at once: the trail never loses a line" |
...
### Phase 4: preview and publish mechanics (4 issues, plus #136, #189, #234 and #94 as they land)
...
| **#257** | Import the older folder-per-class layout, if Windows needs it. If not, one ledger line or an `appliesOn` (Russell's call) | **DECISION** | `referenceCourses.importing.olderLayout`; doc 09 → "The older layout: a folder per class (#254)" |
| **#258** | Import the 2024–25 website-folder layout, if Windows needs it. The Copy a Page half is owed either way, and is done in #247 | **DECISION** | `referenceCourses.importing.quartzCheckoutLayout`; doc 09 → "The 2024–25 layout" |
...
- **The named-gap rule.** A ledger entry is allowed only while an open issue
  ...
  can see. `appliesOn: ["mac"]` is not the way out. The #231 quit exception
  is the one exception, and it ends by deleting both `appliesOn` keys when
  the work is adopted. A scenario cannot be ledgered at all (#260).
...
1. **Before Phase 1: how the ledger is used during this push.** Today the
   rule allows an entry only when the owning issue is on a milestone LATER
   than "the release being cut". This milestone is not a release cut. If
   there is **no Windows release until this milestone closes**, the
   proposal is: ledger entries may name issues ON this milestone while the
   work is in progress; the ledger becomes the burn-down list; add areas for
   `wording` keys, `courseConfigKeys` and `modelTiers.requirements`; and the
   milestone cannot close with any such entry left. The alternative is to
   leave everything red until it is built, and then "did I break anything?"
   cannot be answered for weeks. Also: is a Windows release planned before
   parity? If so, that release's reds are defects, not gaps.
2. **Phase 2: #177.** Should a restore use the lenient shared fence finder
...
4. **Phase 6: #257 and #258.** Does Windows import Russell's two older
   layouts at all? If not: a ledger line or `appliesOn` for each trail event.
5. **Phase 7: #101.** Reserve "work" as a course code in both wizards, or
...
After week one, run Phase 3's #238 before anything else that adds trail
events. Then take Phases 2–4 in whatever interleaving keeps each branch
small. Phase 5 is next, as a chain. Phase 6 waits for #257 and #258.
...
- **Whether work for #180 (or anything else) exists only on the Windows
  machine**, unpushed. And whether `origin/issue/159-settle-the-day-once`
  still merges cleanly into `dev`: nine days and many mac pieces have landed
  on `AssistAgent`-adjacent files since.
