...
**181** after (and still 181 after the fix round): `deployAtATime.sayItAs` (45) is new, with no Windows reader, so
it joins the `deployAtATime` row in the table below. `deployAtATime.asked`
went from 11 to 22, `.refused` from 28 to 51 (`deploy at 6.30 pm` moved to
