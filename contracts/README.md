...
| `gradedFolders.choices` (cases, the depth cap and the skip list) | `GradedFolderChoicesTests`. The mac runs the same list in its own `GradedFolderChoicesTests` since 2026-09-09; the two cases added there that day — the symbol sort and a section folder found deeper down — pass here unchanged, and only the `Count >= 11` guard is worth raising to 13 |
