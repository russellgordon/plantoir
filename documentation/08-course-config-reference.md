...
| `front_page_heading` | string | setup (a club only, #267) | setup only | The heading a new numbered course's section front pages are created with, above the embed of the newest page: a club writes `Most Recent Meeting`. **Absent means `Most Recent Class`.** Read only when a course is created; the assistant's repointing finds the embed by the class page it names, never by the heading, so it never reads or writes this. Not switchable afterwards; Course Settings shows it locked. |
