...
to stop. The build's own give-up path (`_dropping_excluded_items`, which
reconciles the copy lists against `excluded_items` for a config it will not
write) matches exactly since #152, as the preflight drop pass it stands in for
does — it used to lower-case both sides, so one file gave two answers.
