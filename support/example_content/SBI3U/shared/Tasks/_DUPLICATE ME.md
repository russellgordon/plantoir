...
In a science course the observation evidence is usually at the bench: technique,
whether one variable is changed at a time, whether consent is asked, whether a
doubtful reading is retaken or quietly tidied. Name the lab or working period
...
OBSERVE — Unit ?, Day ?, which period
  ...
  finished piece. If you cannot name one, the task may be worth rethinking.
  Going well:
  Stuck:
...
TALK — Unit ?, Day ?, ideally a conference the schedule already has
  Ask: a real question, in the words you would say
  Then: a second one
