...
| 41 | **Half done.** ~~The seventeen `TEACHERS SAY:` phrasings~~ ✅ Done 2026-09-08 — all four tools carry the mac's clause, measured before and after on this PC rather than copied on faith: under test 40/50 → 50/50, controls 55/75 → 60/75 with nothing regressing, overall 76% → 88% (`research/ai-assist/teachers-say-results.txt`, `GUI-IMPROVEMENTS.md` row 451). What is LEFT is the other half: **a rollover publishes over last year's website**, decided 2026-09-08 — it must ASK whether this is a new website or the same one as last year, and leave page visibility alone. Needs the sentence in `contracts/shared-rules.json` (authored, so this side may write it), a second caller for `AssistWorkspace.ReleaseSite`, and the card phrasing to reach something that can ask. `TODO.md` → "A rolled-over section publishes over last year's website". | Medium |
| 42 | Shared Python, found here 2026-09-07: `_dropping_excluded_items` matches `excluded_items` case-insensitively while every other consumer matches exactly, so `build_site.py` gives two answers in one file. **Do not fix it by case-folding the live path** — exact matching is the deliberate rule (`gradedFolders.choices.walk.excludedItems`, reasoned in `GUI-IMPROVEMENTS.md` row 412), and case-folding breaks that case and re-introduces the app/build disagreement 412 rejected. Full write-up in `TODO.md`; `WINDOWS-HANDOFF.md` item 42 (this row said "item 41" until 2026-09-08 — a typo, and item 41 is a different piece of work that now has its own row above). Gateable here now that `dotnet test` runs the shared Python. | Small |
