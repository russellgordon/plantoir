...
| 2026-09-19 | **Hiding a page really hides it.** A page whose `publish:` value a teacher had written on the line BELOW the key (or folded with `>-`) could be reported hidden while students could still read it; asking to hide it could even be a silent no-op. Both are fixed, and the assistant, the sidebar and a new section's inherited visibility now all read a page the way the built site does. | Both — Windows fixed its half first and the mac followed the same design; shared Python moved with it. | [487, 489, 492](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **"Deploy at 6:30 AM" schedules a deploy instead of deploying now.** On the smaller assistant that sentence (it is one of the suggested cards) was read as "deploy" and offered an immediate deploy whose card named no time. It is now understood in code — times such as 6:30 am, 7 pm, 18:30, noon, with today/tomorrow — and the card for an IMMEDIATE deploy now opens by saying it happens now. | **macOS now; Windows owes it ([#193](https://github.com/russellgordon/plantoir/issues/193)).** | [495](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **The assistant no longer stalls on one ordinary sentence.** On the smaller assistant, "publish tomorrow's class and everything it links to" could run for 41–95 seconds and end in an answer that blamed the teacher's wording. Replies are now capped, an unfinished answer runs NOTHING and says so plainly, and asking again does not carry the runaway request with it. About 6 seconds instead. | **macOS now; Windows owes the second half ([#196](https://github.com/russellgordon/plantoir/issues/196))** — it already caps replies. | [496](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **"Duplicate this class as my next class" is safer.** It no longer offers an Undo that would take the copy away but leave the rest of the year's dates moved; the plan says how many later classes really move; the copy starts hidden AND can still be published later; and a lesson already sitting where the copy would go is never written over. | **macOS now; Windows owes its half ([#200](https://github.com/russellgordon/plantoir/issues/200)).** | [497](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **Removing a folder no longer takes marks off the Curriculum Coverage map because of a capital letter** — a course with `Tasks` counted for marks and a teacher's own `Portfolios/tasks` kept its marks only if the spellings matched exactly. | Both — Windows already behaved this way; the rule is now shared. | [491](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **Backup and archive files are named with an ordinary (Gregorian) date on every Mac**, whatever calendar the Mac is set to — and names written by older versions are still read. A teacher whose Mac uses a Buddhist or Japanese calendar no longer gets backups named in that calendar's year, which sorted and pruned wrongly. | macOS — Windows' matching guard landed the same week ([#161](https://github.com/russellgordon/plantoir/issues/161)). | [490, 498](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **Publishing a class no longer publishes the NEXT class early.** If Day 3's page links to Day 4 ("next time we will…"), asking the assistant to publish Day 3 used to publish Day 4 too — and publish and re-date whatever Day 4 linked to. Now a class goes up when the teacher names that class; the plan and the reply say which linked class was left alone, so a link students cannot follow yet is not a surprise. | Both in behaviour — Windows already stopped there; the mac now matches, and the rule is shared. Windows owes the sentence and the test wiring ([#203](https://github.com/russellgordon/plantoir/issues/203), v1.3.0). | [499](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **A request that names ANOTHER course is refused instead of being quietly done on this one.** Typing "publish MCV4U's class" in a VVH2O section's assistant window used to report success — on VVH2O. Now the assistant says which course the window is for and that nothing was done; if the course exists it says to open that course's section, and if no course has that name (a typo) it says so plainly. The section is always this window's, whatever was typed. | **macOS now; Windows owes its half ([#180](https://github.com/russellgordon/plantoir/issues/180), [#208](https://github.com/russellgordon/plantoir/issues/208))** — Windows already refused another course; the two sentences are now shared. | [500](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **"Revise with Codex…"** sits under "Revise with Claude…" in a course's and a section's menu, for a teacher who has OpenAI's Codex installed (it is hidden otherwise; Plantoir never installs either). It opens Codex in the working folder already connected to Plantoir's own tools, so it can list, plan, publish and re-date the way Claude can — and Codex itself asks before each change. Nothing is written outside the teacher's working folder by Plantoir. | **macOS now; Windows later ([#210](https://github.com/russellgordon/plantoir/issues/210), v1.3.1).** | [501](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **The window no longer goes blank after a publish to a folder** (it looked like a crash: no sidebar, no result, only the toolbar — though the publish itself had worked), nor when the note about a publish that ran on its own appears at the top of a section. Both were one fault; clicking another row used to bring the window back. | macOS — Windows builds that panel differently and is asked only to check it ([#214](https://github.com/russellgordon/plantoir/issues/214)). The folder case shipped in v1.1.0; the scheduled-publish note was new in this release and never shipped broken. | [502](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **"Hide" is understood the way "unpublish" is.** "hide unit 4, day 21" (or a whole unit) typed to the assistant now takes the page down, with the same card to approve — on the smaller assistant it used to be handed straight back unanswered. And when the assistant only repeats a request back, it now says it did not follow and the conversation carries on; before, one such reply could leave every later request in that window unanswered until the window was reopened. "publish" is read exactly as it was. | macOS — Windows owes both ([#217](https://github.com/russellgordon/plantoir/issues/217)). Both faults shipped in v1.1.0. | [503](GUI-IMPROVEMENTS.md) |
| 2026-09-19 | **A publish that ran on its own tells you the moment it finishes.** The note appears at the top of the section while you are looking at it, above the site when a preview is showing — it used to wait until the section was left and reopened, and sat mid-window — and the warning beside a section in the sidebar says what it means when the pointer rests on it. Holds for a publish scheduled before updating, too. | macOS — Windows is asked to check its own note ([#218](https://github.com/russellgordon/plantoir/issues/218)). The note is new in this release. | [504](GUI-IMPROVEMENTS.md), [505](GUI-IMPROVEMENTS.md) |
