...
| How a date or title is rewritten when its value continues below the key, and what the site then reads (#199) | `file-formats.json` → `datesAndTitles.writingCases` | `FileFormatsContractTests.testTheDateAndTitleWritersTakeAKeysWholeValue` (14 cases, bytes — the fourteenth, a date running into a line of INDENTED dashes, arrived with [#188](https://github.com/russellgordon/plantoir/issues/188)); `scripts/check_dates_and_titles_against_the_site.py` in the image (verify.sh) checks every `expectSiteReads` against the real `process_frontmatter` |
...
| `wizardAnswerKeys`, `firstDeployMarkers`, `sectionTimetable`, `pageVisibility.writingRules` (the four rule SENTENCES, answered by name) and `pageVisibility.writingCases` — all of them (thirteen until [#188](https://github.com/russellgordon/plantoir/issues/188) made it seventeen; its four are red there until Windows takes #188's fence rule — the `windows` issue from #188), deserialised and played against `PageFrontmatter.SetDraft` since 2026-09-19 ([#138](https://github.com/russellgordon/plantoir/issues/138)), guarded at `>= 10` there so the loop cannot pass having run nothing (the mac's runner carries a floor of its own, raised to 13 with [#176](https://github.com/russellgordon/plantoir/issues/176) and to 17 with #188) | `FileFormatContractTests` |
...
rows to existing lists and no list of their own.
...
| `file-formats.json` → `datesAndTitles.writingCases.cases` | 14 | **Owed**, the `windows` issue opened from [#199](https://github.com/russellgordon/plantoir/issues/199). AUTHORED, arrived 2026-09-25. Windows' `PageFrontmatter.SetTitle` and `SetCreated` replace the key's line alone (`lines[at] = ReplaceRawValue(...)`), so the ten non-control cases fail there until they call their own `ContinuationLines`; the fourteenth ([#188](https://github.com/russellgordon/plantoir/issues/188)) also needs #188's fence rule. Unrun rather than failing. |
